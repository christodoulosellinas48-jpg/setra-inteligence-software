      ...
      {TABS.map((tab) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-medium transition-colors select-none',
          ...
          >
            ...
            {tab.label}
            {active && (
              <span className="absolute bottom-0 w-8 h-0.5 rounded-full bg-[#7B3BFF]" style={{ position: 'absolute', bottom: 'calc(env(safe-area-inset-bottom) + 0px)' }} />
            )}
          </button>
        );
      })}
